(function networkBackground(opts){
  ...
  const cfg = Object.assign({
    ...
    minSpawnDist: 32,                      // px min distance from existing nodes
    spawnAttempts: 6,
    spawnAttemptsFirst: 2,
    linkAlphaMin: 0.14
  }, opts || {});

  const SPAWN = { edgeBias: 0.45, centerBias: 0.20, margin: 40, centerBox: 0.40 };
  ...
  class Node{
    ...
    reset(firstGen){
      ...
      // Choose a spawn location, avoid being too close to others
      ...
      for (let a = 0; a < Math.max(1, attempts); a++){
        ...
        if (!firstGen && r < SPAWN.edgeBias){
          ...
          // Anywhere (also used for firstGen)
          candX = rand(0, w);
          candY = rand(0, h);
          candAng = rand(0, Math.PI * 2);
        }

        if (!tooClose(candX, candY)) break; // accept candidate
        if (a === attempts - 1) break;      // give up, use last candidate
      }

      this.x = candX; this.y = candY; ang = candAng;

      if (edgeSpawn){
        // Ensure life long enough to cross toward the center
      ...
      if (firstGen){
        this.t = rand(0, this.life); // staggered starts
      } else {
    ...
    alpha(){
      ...
      let aIn = 1, aOut = 1;
      if (p < fi)     aIn  = ease(p / fi);
      if (p > 1 - fo) aOut = ease((1 - p) / fo);
    ...
    drawDot(ctx){
      ...
      const glow = (base, mult) => base.replace(/,?[^,]+?\)$/, ',' + mult.toFixed(3) + ')');
      g.addColorStop(0, glow(cfg.glowColor, 0.6 * a));
...
})(window.NETWORK_OPTS || {}); // read optional page-level overrides

// --- Optional: manifesto typewriter (safe if element absent) ---------------
document.addEventListener('DOMContentLoaded', () => {
  ...
  details.addEventListener('toggle', () => {
    if (details.open) start(); else { cancel(); resetDisplay(); }
  });
